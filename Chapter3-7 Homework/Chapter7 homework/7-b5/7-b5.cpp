...
int stu_list::print(const char* prompt)
{
	...
	for (int i = 0; i < list_merge_num; i++) {
		int name_len = strlen(list_merge[i].stu_name);
	...
	for (int i = 0; i < list_merge_num; i++) {
		...
		cout << " " << setw(4) << (i + 1);
		cout << "  " << setw(7) << list_merge[i].stu_no;
		cout << "  " << setw(max_name_len) << list_merge[i].stu_name;
		if (list_merge[i].round1 && list_merge[i].round2) {
			cout << "  " << setw(6) << "Y";
			cout << "  " << setw(6) << "Y";
		}
		else if (list_merge[i].round1 && !list_merge[i].round2) {
			cout << "  " << setw(6) << "Y";
			cout << "  " << setw(6) << "退课";
		}
		else if (!list_merge[i].round1 && list_merge[i].round2) {
...
void stu_list::merge_lists()
{
	...
	for (int i = 0; i < list_num_1; i++) {
		...
		list_merge[list_merge_num].round1 = true;   // 参加了第一轮
		list_merge[list_merge_num].round2 = false;  // 默认没参加第二轮
		list_merge_num++;
	}
	for (int i = 0; i < list_num_2; i++) {
		bool found = false;
		for (int j = 0; j < list_merge_num; j++) {
			if (list_merge[j].stu_no == list_round_2[i].no) {
				list_merge[j].round2 = true;
				found = true;
				break;
			}
		}
		if (!found) {
			...
			list_merge[list_merge_num].round1 = false;  
			list_merge[list_merge_num].round2 = true;  
			list_merge_num++;
		}
	}
}
...
/***************************************************************************
  ...
  说    明：冒泡排序
***************************************************************************/
void stu_list::sort_merged_list()
{
	if (list_merge_num <= 1)
		return;
	for (int i = 0; i < list_merge_num - 1; i++) {
		for (int j = 0; j < list_merge_num - i - 1; j++) {
			if (list_merge[j].stu_no > list_merge[j + 1].stu_no) {
				...
				strcpy(temp.stu_name, list_merge[j].stu_name);
				...
				list_merge[j].round1 = list_merge[j+1].round1;
				list_merge[j].round2 = list_merge[j+1].round2;
				list_merge[j + 1].stu_no = temp.stu_no;
				strcpy(list_merge[j+1].stu_name, temp.stu_name);
				list_merge[j + 1].round1 = temp.round1;
				list_merge[j + 1].round2 = temp.round1;
			}
		}
	}
}
